    ...
    <div class="flex flex-col justify-start items-center h-screen">
        ...
        <ul class="flex justify-center items-center my-4 font-sans font-medium">
            <div v-for="(tab, index) in tabs" :key="index">
                <li class="cursor-pointer py-2 px-4 text-gray-500 border-b-2" :class="activeTab === index ? 'text-teal-700 border-teal-700' : ''" @click="activeTab = index" v-text="tab"></li>
            </div>
        </ul>
            ...
            <div v-show="activeTab === 0">
                <form action="POST" @submit.prevent="userSignIn">
                    <div class="bg-white rounded-xl w-full max-w-sm">
                        ...
                        <button class="mt-4 w-full bg-teal-500 hover:bg-teal-700 focus:ring focus:ring-teal-100 text-white py-2 rounded-md text-lg tracking-wide">Sign In</button>
                    </div>
                </form>
            </div>
            <div v-show="activeTab === 1">
                <form action="POST" @submit.prevent="userSignUp">
                    <div class="bg-white rounded-xl w-full max-w-sm">
                        ...
                        <button class="mt-4 w-full bg-teal-500 hover:bg-teal-700 focus:ring focus:ring-teal-100 text-white py-2 rounded-md text-lg tracking-wide">Sign Up</button>
                    </div>
                </form>
            </div>
        </div>
