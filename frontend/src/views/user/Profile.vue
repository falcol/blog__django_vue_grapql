<template>
    <div class="flex flex-col">
        <div>
            <img :src="'/uploads/' + this.userInfo.avatar" alt="" class="w-32 h-3w-32 rounded-lg object-contain mx-auto" />
        </div>
        <div class="grid grid-col-1 md:grid-cols-2 gap-4 my-24 border-2 p-10">
            <div class="md:self-center md:px-4 md:space-y-2">
                <p class="font-serif text-5xl">{{ this.userInfo.firstName }} {{ this.userInfo.lastName }}</p>
                ...
                <div class="relative px-4 w-full max-w-2xl h-full md:h-auto">
                    <div class="relative bg-white rounded-lg shadow-lg border-2">
                        <div class="flex justify-between items-start p-5 rounded-t border-b">
                            <h3 class="text-xl font-semibold text-gray-900 lg:text-2xl dark:text-white">Update Your Profile</h3>
                            <button
                                type="button"
                        ...
                        <form action="POST" enctype="multipart/form-data" @submit.prevent="updateProfile">
                            <div class="p-6 space-y-6">
                                <div class="grid grid-cols-2 gap-2">
                                    <div>
                                        <label for="firstName" class="block mb-1 text-gray-600 font-medium">First Name</label>
                                        <input
                                    ...
                                    <div>
                                        <label for="lastName" class="block mb-1 text-gray-600 font-medium">Last Name</label>
                                        <input
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <div>
                                        <label for="formFile" class="block mb-1 text-gray-600 font-medium">Avatar</label>
                                        <input
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <div>
                                        <label for="location" class="block mb-1 text-gray-600 font-medium">Location</label>
                                        <input
                                    ...
                                    <div>
                                        <label for="website" class="block mb-1 text-gray-600 font-medium">Website</label>
                                        <input
                                ...
                                <div>
                                    <label for="bio" class="block mb-1 text-gray-600 font-medium">Bio</label>
                                    <textarea
        ...
        <div class="border-2 p-10 space-y-10 divide-y-2">
            ...
            <div v-for="comment in userInfo.commentSet" :key="comment.id" class="py-4">
                <h3 class="font-sans font-bold text-lg">
                    <a :href="`/post/${comment.post.slug}`">{{ comment.post.title }}</a> - <span v-if="comment.isApproved" class="text-teal-500">Approved</span
...
export default {
    ...
    methods: {
        async updateProfile() {
            ...
            localStorage.setItem('user', JSON.stringify(user.data.updateUserProfile.user))
