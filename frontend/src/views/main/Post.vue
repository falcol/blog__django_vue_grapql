<template>
    <div class="home">
        ...
        <div class="flex flex-wrap my-4">
            ...
            <router-link v-for="tag in this.postBySlug.tag" :key="tag.name" class="mr-5 text-sm font-medium uppercase text-teal-500 hover:underline hover:text-teal-700" :to="`/tag/${tag.slug}`">{{
                tag.name
            }}</router-link>
        </div>
        ...
        <div class="flex flex-wrap py-4 space-x-8 justify-center items-center text-xl">
            <div v-if="this.liked === true" @click="this.updateLike()">
                <i class="fa-solid fa-thumbs-up">
                    <span class="font-sans font-semibold ml-1">{{ this.numberOfLikes }}</span>
                </i>
            </div>
            ...
            <div @click="this.toggleCommentSection()">
                <i class="fa-regular fa-comment-dots"
                    ><span class="font-sans font-semibold ml-1">{{ this.numberOfApprovedComments }}</span></i
                >
            </div>
        ...
        <comment-section-component v-if="this.approvedComments && this.showComment" :comments="this.approvedComments" :postID="this.postBySlug.id" :userID="this.userID"></comment-section-component>
    </div>
</template>
