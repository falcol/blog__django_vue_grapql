<template>
    <div class="flex flex-col my-4 py-4 space-y-10 border-t-2">
        ...
        <div v-if="!this.user.isAuthenticated">
            You need to
            <router-link to="/account" class="text-teal-500 hover:underline hover:text-teal-700">sign in</router-link>
            before you can leave your comment.
        </div>

        <!-- If the user is authenticated -->
        <div v-else>
            <div v-if="this.commentSubmitSuccess" class="">Your comment will show up here after is has been approved.</div>
            <form action="POST" @submit.prevent="submitComment">
                ...
                <button class="mt-4 w-full bg-teal-500 hover:bg-teal-700 focus:ring focus:ring-teal-100 text-white py-2 rounded-md text-lg tracking-wide">Submit Comment</button>
            </form>
        </div>

        <!-- List all comments -->
        <comment-single v-for="comment in comments" :key="comment.id" :comment="comment" :userID="this.userID"> </comment-single>
    </div>
</template>
