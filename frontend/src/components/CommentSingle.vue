<template>
    <div class="border-2 p-4">
        <div class="flex flex-row justify-start content-center items-center space-x-2 mb-2">
            <img :src="`http://127.0.0.1:8000/media/${this.comment.user.avatar}`" alt="" class="w-10" />
            <p class="text-lg font-sans font-bold">
                {{ this.comment.user.username }}
            </p>
            <p class="text-sm text-gray-500">- {{ formatDate(this.comment.createdAt) }}</p>
        </div>
        ...
        <div class="flex flex-wrap py-4 space-x-2 items-center">
            ...
            <div v-if="this.liked === true" @click="this.updateLike()">
                <i class="fa-solid fa-thumbs-up">
                    <span class="font-sans text-sm font-normal ml-1">{{ this.numberOfLikes }}</span>
                </i>
            </div>
